...
function LoginForm () {
    ...
    const handleLogin = async e => {
        e.preventDefault()
        if(validate()){
            const resp =  await fetch(`https://ccserver-obi1.onrender.com/creators/${userName}`)
            const loginAttempt = await resp.json()
            if(loginAttempt.pass === pass)
    ...
    if(user)
        return <div>
            ...
            <p>Would you like to go to your <Link to={`creator/${user.id}`}>profile</Link> or log out?</p>
        </div>
    else
        return <form id="loginForm" onSubmit={handleLogin}>
